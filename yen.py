...
#
# @retval Array of paths, where [0] is the shortest, [1] is the next shortest, 
# and so on.
        ...
        for i in range(0, len(A[-1]['path']) - 1):
            ...
            for path_k in A:
                if path_root == path_k['path'][:i+1]:
                    cost = graph.removeLink(path_k['path'][i], path_k['path'][i+1])
                    if cost == -1:
                        continue
                    edges_removed.append([path_k['path'][i], path_k['path'][i+1], cost])
